...
- `/`는 저장된 명시적 선택과 브라우저 언어를 기기 안에서만 확인해 가장
  적합한 언어 링크를 강조하는 `x-default` 진입점이며, 실제 언어 페이지는
  `/en/`, `/vi/`, `/id/`다. 검색 크롤러와 사용자를 자동 이동시키지 않는다.
- 지원하지 않는 브라우저 언어와 감지 실패는 영어를 추천한다. 지원 언어의
  누락 문구를 영어로 조용히 대체하는 동작은 허용하지 않는다.
- 모든 공개 페이지에 텍스트 언어 전환, 언어별 metadata, self-canonical,
  상호 `hreflang`, `x-default`를 제공하고 검색 색인을 허용한다. 404만
  `noindex, nofollow`를 유지한다.
- 루트 sitemap과 robots sitemap 선언을 제공하고, 루트에는 실제 공개 정보만
  담은 `WebSite` 구조화 데이터를 제공한다. 미출시 강의·상품·평점은 선언하지 않는다.
...
1. `vi-*`, `id-*`, `en-*` 브라우저 언어와 유효한 저장 선택을 allowlist로
   결정해 해당 링크를 추천하며 그 밖의 값은 영어 추천으로 안전하게 수렴한다.
